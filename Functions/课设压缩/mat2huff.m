...
% Y = MAT2HUFF(X) Huffman encodes matrix x using symbol probabilities in
% unit-width histogram bins between X's minimum and maximum values. The
% encoded data is returned as a structure Y:
%   Y.code   The Huffman.encoded values of X, stored in a uint16 vector.
...
%
% If X is double with non-integer value---for example, an image with values
% between 0 and 1---first scale X to an appropriate integer range before
% the call. For example, use Y = MAT2HUFF(255*X) for 256 gray level
% encoding.
% NOTE: The number of Huffman code words is
% round(max(X(:)))-round(min(X(:)))+1. You may need to scale input X to
% generate codes of reasonable length. The maximum row or column dimension
% of X is 65535.
%
% See also HUFF2MAT
