...
%
% Based on huffman5 by Sean Danaher, University of Northumbria, Newcastle
% UK. Available at the MATLAB Central File Exchange: Category General DSP
...
while numel(s) > 2
    [p, i] = sort(p); % Sort the symbol probabilities. 递增排序
                      % p is the sorted array and i is the index array
...
%celldisp(s); %递归地显示单元数组内容
%cellplot(s); %产生类似嵌套框的单元数组的图形描述
